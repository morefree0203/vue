...
function submitCourse() {
  ...
  if (editForm.id) {
    axios.put('http://localhost:8080/api/course/update', editForm)
      .then(() => {
    ...
    axios.post('http://localhost:8080/api/course/add', editForm)
      .then(() => {
...
function deleteCourse(id) {
  ElMessageBox.confirm('确定要删除该课程吗？', '提示', { type: 'warning' })
    .then(() => {
      axios.delete(`http://localhost:8080/api/course/delete/${id}`)
        .then(() => {
