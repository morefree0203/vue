<template>
  <div class="department-course-management">
...
function fetchCourses() {
  axios.get('http://localhost:8080/api/course/department-list')
    .then(res => {
...
function submitCourse() {
  ...
  if (editForm.id) {
    axios.put('http://localhost:8080/api/course/department-update', editForm)
      .then(() => {
    ...
    axios.post('http://localhost:8080/api/course/department-add', editForm)
      .then(() => {
...
function deleteCourse(id) {
  ElMessageBox.confirm('确定要删除该课程吗？', '提示', { type: 'warning' })
    .then(() => {
      axios.delete(`http://localhost:8080/api/course/department-delete/${id}`)
        .then(() => {
