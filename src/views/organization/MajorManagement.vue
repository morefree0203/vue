...
const fetchMajors = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/organization/major/list')
...
const submitForm = () => {
  formRef.value.validate(async (valid) => {
    ...
    try {
      if (form.id) {
        // 编辑
        await axios.post('/api/organization/major/update', form)
        ElMessage.success('编辑成功')
      } else {
        // 新增
        await axios.post('/api/organization/major/add', form)
...
const deleteMajor = (id) => {
  ElMessageBox.confirm('确定删除该专业吗？', '提示', { type: 'warning' })
    .then(async () => {
      await axios.post('/api/organization/major/delete', { id })
