...
// 辅助函数：根据角色和指标名称获取最大分数
const getMaxScore = (roleType, indicatorName) => {
  if (!props.evaluationData.maxScores || !Array.isArray(props.evaluationData.maxScores)) {
    return 10 // 默认值
  }
  const maxScoreItem = props.evaluationData.maxScores.find(
    item => item.roleType === roleType && item.indicatorName === indicatorName
  )
  return maxScoreItem ? maxScoreItem.maxScore : 10 // 默认值
}
...
const supervisorAvgScores = computed(() => {
  ...
  return Object.keys(avgScores).map(indicatorName => ({
    ...
    maxScore: getMaxScore('Supervisor', indicatorName)
  }))
})
...
const peerAvgScores = computed(() => {
  ...
  return Object.keys(avgScores).map(indicatorName => ({
    indicatorName,
    avgScore: avgScores[indicatorName],
    maxScore: getMaxScore('Peer', indicatorName)
  }))
})
...
const leaderAvgScores = computed(() => {
  ...
  return Object.keys(avgScores).map(indicatorName => ({
    indicatorName,
    avgScore: avgScores[indicatorName],
    maxScore: getMaxScore('Leader', indicatorName)
  }))
})
...
const studentAvgScores = computed(() => {
  ...
  return Object.keys(avgScores).map(indicatorName => ({
    indicatorName,
    avgScore: avgScores[indicatorName],
    maxScore: getMaxScore('Student', indicatorName)
  }))
})
